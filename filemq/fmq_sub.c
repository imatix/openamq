...
#define CLIENT_NAME "FileMQ-Subscriber/0.1"
...
"warranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.\n" \
    "\n"
#define USAGE                                                               \
    "Syntax: fmq_sub [options...]\n"                                        \
    "Options:\n"                                                            \
    "  -n servicename   Service name (filemq)\n"                            \
    "  -c channel       Channel name or pattern (#)\n"                      \
...
#include "fmq_sub.d"                    //  Include dialog data
...
static char
    ...
    *opt_channel,                       //  Channel name or pattern
...
#   include "fmq_sub.i"                 //  Start dialog engine
}
...
MODULE initialise_the_program (void)
{
    ...
    icl_console_print ("I: starting up file subscriber, service=%s", opt_service);
...
MODULE open_connection_to_broker (void)
{
    ...
    while (token) {
        connection = amq_client_connection_new (
            token->value, "/", auth_data, "fmq_sub", atoi (opt_trace), 30000);
        if (connection) {
...
MODULE register_as_subscriber (void)
{
    ...
    s_check_status (rc, "E: can't declare private subscriber queue");
    ...
    if (!exception_raised) {
        ...
        s_check_status (rc, "E: can't bind subscriber queue '%s'", queue_name);
    }
    if (!exception_raised) {
        ...
        s_check_status (rc, "E: can't consume from subscriber queue '%s'", queue_name);
    }
}
...
{
    icl_console_print ("I: shutting down file subscriber");
}
