...
using log4net;
using minadotnet.jpmorgan.mina.filter.codec.demux;
...
namespace jpmorgan.mina.filter.codec.demux
{
    public class DemuxingProtocolCodecFactory : IProtocolCodecFactory
    {
        ...
        protected void DisposeCodecResources(ISession session)
        {
        ...
        private class ProtocolEncoderImpl : IProtocolEncoder
        {
            ...
            public void Encode(ISession session, object message,
            ...
            {
                _enclosing.DisposeCodecResources(session);
            }
        }
        ...
        private class ProtocolDecoderImpl : CumulativeProtocolDecoder
        {
            ...
            public ProtocolDecoderImpl(DemuxingProtocolCodecFactory enclosing) : base(16)
            {
                _enclosing = enclosing;
            ...
            protected override bool DoDecode(ISession session, ByteBuffer input,
                                             IProtocolDecoderOutput output)
            {
                MessageDecoderResult result;
                if (_currentDecoder == null)
                {
                    IMessageDecoder[] decoders = this._decoders;
                    int undecodables = 0;
                ...
                result = _currentDecoder.Decode(session, input, output);
                if (result == MessageDecoderResult.OK)
                {
