...
package AMQP::Framing::Frames::StreamConsume;
...
sub write_method($) {
    ...
    $self->{'codec'}->write_bit(($self->{'no_local'}, $self->{'exclusive'}, $self->{'nowait'}));
}
...
sub read_method($) {
    ...
    ($self->{'no_local'}, $self->{'exclusive'}, $self->{'nowait'}) = $self->{'codec'}->read_bit();
}
