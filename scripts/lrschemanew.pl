...
use warnings;
BEGIN {
    require '$source.d';                #   Include dialog interpreter
    main_dialog->import ();
};
...
BEGIN {
    ...
    $$VERSION    = $VERSION;
    @ISA         = qw(Exporter);
    # Export public subroutines and event constants
    @EXPORT      = qw(&execute &raise_exception
                      $$the_next_event $$exception_event $$exception_raised
:do event
                      $$$name
:enddo
                      $$terminate_event
...
our ($$LR_event);                       # Event for state transition 
our ($$LR_state);                       # Current dialog state
our ($$LR_savest);                      # Saved dialog state
our ($$LR_index);                       # Index of methods function

our ($$the_next_event);                 # Next event from module
our ($$the_exception_event);            # Exception event from module
our ($$exception_raised);               # TRUE if exception raised
our ($$exit_status);                    # Exit status of application

#-- Symbolic constants and event numbers ------------------------------------

:if defaults
our $$LR_defaults_state;
...
# Useful constants for exit status of application
our ($$EXIT_SUCCESS) = 0;
our ($$EXIT_FAILURE) = 255;

#-- Private variables used by dialog interpreter ----------------------------
 
our ($$LR_nextst, $$LR_action, $$LR_module);
our (@LR_ename, @LR_mname, @LR_sname, @LR_offset);
:if module "Dialog-Call"
our (@LR_stack, $$LR_stack_size);
:endif
...
:set array_base = 1
:do module
            $$LR_modnbr == $number && do { &main::$name; last; };
:enddo
:set array_base = 0
          }
        $$LR_vecptr++;
